...
struct segment_tree_lazy {
	...
	void push(int n, int b, int e) {
		if (lazy[n] == 0) return;
		t[n].summ = t[n].summ + (TT) lazy[n] * (e - b + 1);
...
int32_t main() {
	...
	// 1-index based
	segment_tree_lazy<node> seg(n);       // empty segment tree of n size
	segment_tree_lazy<node> seg_arr(arr); // segment tree using arr

	seg.update(1, 3, 5);                      // add 5 to each element from index 1 to 3
